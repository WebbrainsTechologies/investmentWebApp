...
import { getInvoiceList, getInvoiceListByUser } from "services/invoiceServices";
...
const UserInvoiceList = props => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableFilters: true,
        accessor: "userSubscriptionDetails.name",
        Cell: tableInstance => (
          <span>
            {tableInstance.row.original.userSubscriptionDetails?.name}
          </span>
        )
      },
      {
        ...
        Cell: tableInstance => (
          <span>
            {tableInstance.row.original?.userSubscriptionDetails?.amount}
          </span>
        )
      },
      {
  ...
  const getServiceHandler = useCallback(async () => {
    ...
    setloader(true);
    await getInvoiceListByUser(token, data)
      .then(res => {
