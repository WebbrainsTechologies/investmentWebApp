...
// import GradesAddModal from "./gradesAddModal";
// import loader_Img from "../../../assets/images/images/Loaders/loader-1.svg";
...
const SubscriptionList = props => {
  ...
  const updateStatus = async data => {
    ...
    await changeSubscriptionStatus(token, data.id, {
      status: data.status
    }).then(data => {
      if (data.success) {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableFilters: true,
        accessor: "currency",
        Cell: tableInstance => <span>{tableInstance.row.values.currency}</span>
      },
      {
        Header: tableInstance => {
          return (
        ...
        disableFilters: true,
        accessor: "amount",
        Cell: tableInstance => <span>{tableInstance.row.values.amount}</span>
      },
      {
        Header: tableInstance => {
          return (
        ...
        Cell: tableInstance => (
          <span>
            {tableInstance.row.values.duration === 1
              ? `${tableInstance.row.values.duration} Month`
              : `${tableInstance.row.values.duration} Months`}
          </span>
        )
      },
      {
        ...
        // Filter: FilterComponent,
        placeholder: "ROI Duration",
        disableFilters: true,
        disableSortBy: true,
        accessor: "roi_duration",
        Cell: tableInstance => (
          <span className="text-capitalize">
            {tableInstance.row.values.roi_duration}
          </span>
        )
      },
      {
            ...
            <div className="react-action-class">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setSubscriptionName(tableInstance.row.original?.name);
                }}
              >
                <Trash2 />
              </button>
  ...
  const getServiceHandler = useCallback(async () => {
    ...
    setloader(true);
    await getSubscriptionList(token, data)
      .then(res => {
