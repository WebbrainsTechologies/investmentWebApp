...
import {
  getCommisionPayout,
  walletWithdrawalrequestforuser
...
const CommisionPayout = props => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableFilters: true,
        accessor: "acctual_package_amount",
        Cell: tableInstance => (
          <span>
            {tableInstance.row.values?.acctual_package_amount?.toFixed(2)}
          </span>
        )
      },
      {
        ...
        Cell: tableInstance => (
          <span>
            {tableInstance.row.original?.userDetails.name
              ? tableInstance.row.original?.userDetails.name
              : "-"}
          </span>
        )
  ...
  const getServiceHandler = useCallback(async () => {
    ...
    setloader(true);
    await getCommisionPayout(token, data)
      .then(res => {
