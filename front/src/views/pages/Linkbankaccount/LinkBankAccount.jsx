...
import NameMatchingTooltip from "./InfoBtn";
...
const Linkbankaccount = props => {
  ...
  const editpage = queryParams.get("editpage");
  const id = queryParams.get("id");
  // console.log(id, "checkid35");
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      intervalCheck &&
        (await checkbankstatus(useracessTokenByOnMeta, referenceNumber).then(
          async res => {
            ...
            if (res.success) {
              ...
              if (bankstatus === "SUCCESS") {
                ...
                formData.append("is_bank_verified", true);
                setLoading(true);
  ...
  return (
    <div>
      {user?.is_bank_verified && bankDetails["accountNumber"] ? (
          ...
          <div className="row">
            <div className="col-sm-6">
              <div className="form-group">
                <label>Name</label> <span className="red">*</span>{" "}
                <span>
                  <NameMatchingTooltip
                    message={
                      "Your name must match with the Aadhar card and PAN card name."
                    }
                  />
                </span>
                <input
