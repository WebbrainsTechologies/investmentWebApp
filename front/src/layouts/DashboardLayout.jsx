...
import { getUserAllNotifications } from "services/userNotificationServices";
import navigationAction from "redux/navigation/actions";
const { changeTheme } = themeActions;
const { sidebarMini } = settingactions;
const { getNotificationData, getKycStatus } = navigationAction;

const DashboardLayout = props => {
  const location = useLocation();
  const { token, getNotificationData, getKycStatus, user } = props;
  ...
  const { layoutTheme, themeSetting } = props;
  ...
  const userNotifications = async () => {
    await getUserAllNotifications(token).then(data => {
      // console.log(data.data, "check126");
  ...
  useEffect(() => {
    token && getNotificationData(token);

    // eslint-disable-next-line
  }, [location.pathname]);

  useEffect(() => {
    user && token && getKycStatus(token, user?._id);
  }, [location.pathname]);
  // console.log(notifications, "checknotifications1134");
  return (
    <IntlProvider
      locale="en"
